...
var $author$project$Main$clearScreen = A2(
	...
	_List_fromArray(
		[
			$joakin$elm_canvas$Canvas$Settings$fill($avh4$elm_color$Color$darkPurple)
		]),
	_List_fromArray(
		[
			A3(
...
var $author$project$Main$SubSquares = F2(
	function (size, squares) {
		return {size: size, squares: squares};
	});
var $author$project$Main$renderSieRemoved = function (removedSub) {
	var squars = removedSub.squares;
	var size = removedSub.size;
	return A2(
		$elm$core$List$map,
		function (squar) {
			return A3($joakin$elm_canvas$Canvas$rect, squar, size, size);
		},
		squars);
};
...
var $author$project$Main$sub_squares = function (subsquare) {
	var squares = subsquare.squares;
	var size = subsquare.size;
	var subSize = size / 3;
	var sub_square = function (position) {
		var y = position.b;
		var x = position.a;
		var midy = y + subSize;
		var midx = x + subSize;
		var endy = y + (subSize * 2);
		var endx = x + (subSize * 2);
		return _Utils_Tuple2(
			_Utils_Tuple2(midx, midy),
			_List_fromArray(
				[
					_Utils_Tuple2(x, y),
					_Utils_Tuple2(midx, y),
					_Utils_Tuple2(endx, y),
					_Utils_Tuple2(x, midy),
					_Utils_Tuple2(endx, midy),
					_Utils_Tuple2(x, endy),
					_Utils_Tuple2(midx, endy),
					_Utils_Tuple2(endx, endy)
				]));
	};
	var sub = $elm$core$List$unzip(
		A2($elm$core$List$map, sub_square, squares));
	var removed = sub.a;
	var newSub = $elm$core$List$concat(sub.b);
	return _Utils_Tuple2(
		A2($author$project$Main$SubSquares, subSize, removed),
		A2($author$project$Main$SubSquares, subSize, newSub));
};
var $author$project$Main$sieRemoved = F2(
	function (iteration, size) {
		var helper = F3(
			function (iteration1, suSquare, cutout) {
				helper:
				while (true) {
					if (iteration1 <= 0) {
						return cutout;
					} else {
						var iter = $author$project$Main$sub_squares(suSquare);
						var $temp$iteration1 = iteration1 - 1,
							$temp$suSquare = iter.b,
							$temp$cutout = A2($elm$core$List$cons, iter.a, cutout);
						iteration1 = $temp$iteration1;
						suSquare = $temp$suSquare;
						cutout = $temp$cutout;
						continue helper;
					}
				}
		...
		return A2(
			$elm$core$List$map,
			$author$project$Main$renderSieRemoved,
			A3(
				helper,
				iteration,
				A2(
					$author$project$Main$SubSquares,
					size,
					_List_fromArray(
						[
							_Utils_Tuple2(0, 0)
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						A2($elm$html$Html$Attributes$style, 'border', '8px solid rgba(0,0,0,0.1)')
					]),
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$joakin$elm_canvas$Canvas$Settings$fill($avh4$elm_color$Color$lightPurple)
							]),
						$elm$core$List$concat(
							A2($author$project$Main$sieRemoved, model, 600.0)))
					]))
			]));
};
